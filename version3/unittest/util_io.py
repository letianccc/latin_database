...
def read_integer(f):
    ...
    INTEGER_LENGTH = IntType.get_size()
    try:
        byte = f.read(INTEGER_LENGTH)
        byte = pickle.loads(byte)
...
def write_string(f, string, max_string_size=None):
    ...
    f.write(byte)
    if max_string_size:
        pad_size = max_string_size - len_ - IntType.get_size()
        write_pad(f, pad_size)
        # print(len_, IntType.get_size(), pad_size, max_string_size)
